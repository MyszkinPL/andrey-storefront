...
export function AlertDialogMedia({
  ...
  return (
    <div
      className={cn(
        "mb-1 flex size-10 shrink-0 items-center justify-center rounded-full bg-muted text-foreground [&_svg:not([class*='size-'])]:size-5",
        className,
      )}
...
/**
 * Confirm button. Deliberately not an AlertDialogClose: callers render a
 * pending state inside it while a mutation runs, so closing is left to the
 * caller's own open state.
 */
